...
$checks = [
    ...
    strpos($install, "(\$_POST['licence_agree'] ?? '') !== 'Y'") !== false,
    strpos($install, '$_REQUEST') === false,
    strpos($install, 'getRequestStep') !== false,
    strpos($install, 'array_key_exists(\'step\', $_GET)') !== false,
    strpos($install, 'array("DEFAULT" => "Y")') !== false,
    strpos($install, 'array("ACTIVE" => "Y")') !== false,
